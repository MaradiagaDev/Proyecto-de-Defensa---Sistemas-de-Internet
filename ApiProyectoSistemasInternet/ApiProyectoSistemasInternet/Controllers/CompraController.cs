...
namespace ApiProyectoSistemasInternet.Controllers
{
    ...
    public class CompraController : ControllerBase
    {
            ...
            try
            {
                ...
                if (!response.isExitoso || response.resultado == null)
                {
                    return NotFound(new
                    {
                        statusCode = 404,
                        isExitoso = false,
                        errorMessages = response.errorMessages ?? new List<object> { "Compra no encontrada." }
                    });
                }
        ...
        [HttpPost]
        public IActionResult UpdateCreateCompra([FromBody] Compra compra)
        {
            try
            {
                var identity = HttpContext.User.Identity as ClaimsIdentity;
                ...
                var response = _repository.UpdateCreateObject(compra);

                if (response.isExitoso)
                {
                    if (response.statusCode == 201)
                    {
                        return CreatedAtAction(nameof(GetById), new { id = response.resultado.IdCompra }, response.resultado);
                    }
        ...
        public IActionResult GetAllObjects([FromQuery] int offSet, [FromQuery] int pageSize)
        {
            ...
            {
                return StatusCode(500, new GetAllResponse<Compra>
                {
                    statusCode = 500,
                    isExitoso = false,
        ...
        public IActionResult DeleteObject(int id)
        {
            try
            {
                ...
                if (result)
                {
                    return StatusCode(200, new GetAllResponse<Compra>
                    {
                        statusCode = 200,
                        isExitoso = true,
