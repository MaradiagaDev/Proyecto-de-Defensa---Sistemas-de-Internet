...
namespace ApiProyectoSistemasInternet.Controllers
{
    ...
    {
        private readonly EmpleadoRepository _repository = new EmpleadoRepository();
            ...
            try
            {
                ...
                if (!response.isExitoso || response.resultado == null)
                {
                    return NotFound(new
                    {
                        statusCode = 404,
                        isExitoso = false,
                        errorMessages = response.errorMessages ?? new List<object> { "Empleado no encontrado." }
                    });
                }
        ...
        [HttpPost]
        public IActionResult UpdateCreateUser([FromBody] Empleado empleado)
        {
            try
            {
                var identity = HttpContext.User.Identity as ClaimsIdentity;
                ...
                var response = _repository.UpdateCreateObject(empleado);

                if (response.isExitoso)
                {
                    if (response.statusCode == 201)
                    {
                        return CreatedAtAction(nameof(GetById), new { id = response.resultado.IdEmpleado }, response.resultado);
                    }
