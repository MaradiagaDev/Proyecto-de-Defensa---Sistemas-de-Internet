using ApiProyectoSistemasInternet.DTO;
...
namespace ApiProyectoSistemasInternet.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public IActionResult Login([FromBody] UserDto userDto)
        {
            try
            {
                // Autenticamos al usuario y generamos el token JWT
                var token = userDto.AuthenticateAsync();
            ...
            try
            {
                ...
                if (!response.isExitoso || response.resultado == null)
                {
                    return NotFound(new
                    {
                        statusCode = 404,
                        isExitoso = false,
                        errorMessages = response.errorMessages ?? new List<object> { "Usuario no encontrado." }
                    });
                }
        ...
        [HttpPost]
        public IActionResult UpdateCreateUser([FromBody] Usuario usuario)
        {
            try
            {
                var identity = HttpContext.User.Identity as ClaimsIdentity;
                ...
                var response = _repository.UpdateCreateObject(usuario);

                if (response.isExitoso)
                {
                    if (response.statusCode == 201)
                    {
                        return CreatedAtAction(nameof(GetById), new { id = response.resultado.IdUsuario }, response.resultado);
                    }
        ...
        public IActionResult GetAllObjects([FromQuery] int offSet, [FromQuery] int pageSize)
        {
            ...
            {
                // Si ocurre una excepción, devolvemos el error con un mensaje general
                return StatusCode(500, new GetAllResponse<Usuario>
                {
                    statusCode = 500,
                    isExitoso = false,
        ...
        public IActionResult DeleteObject(int id)
        {
            try
            {
                ...
                if (result)
                {
                    return StatusCode(200, new GetAllResponse<Usuario>
                    {
                        statusCode = 200,
                        isExitoso = true,
