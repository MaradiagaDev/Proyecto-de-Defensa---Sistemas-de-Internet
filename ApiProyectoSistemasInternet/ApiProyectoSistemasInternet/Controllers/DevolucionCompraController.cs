...
namespace ApiProyectoSistemasInternet.Controllers
{
    ...
    {
        private readonly DevolucionCompraRepository _repository = new DevolucionCompraRepository();
        ...
        public IActionResult GetById(int id)
        {
            try
            {
                ...
                // Obtener la Devolución de compra por ID
                ...
                if (!response.isExitoso || response.resultado == null)
                {
                    return NotFound(new
                    {
                        statusCode = 404,
                        isExitoso = false,
                        errorMessages = response.errorMessages ?? new List<object> { "Devolución de compra no encontrada." }
                    });
                }
        ...
        [HttpPost]
        public IActionResult UpdateCreateDevolucionCompra([FromBody] DevolucionCompra devolucionCompra)
        {
            try
            {
                var identity = HttpContext.User.Identity as ClaimsIdentity;
                ...
                var response = _repository.UpdateCreateObject(devolucionCompra);

                if (response.isExitoso)
                {
                    if (response.statusCode == 201)
                    {
                        return CreatedAtAction(nameof(GetById), new { id = response.resultado.IdDevolucionCompra }, response.resultado);
                    }
        ...
        public IActionResult GetAllObjects([FromQuery] int offSet, [FromQuery] int pageSize)
        {
            ...
            {
                return StatusCode(500, new GetAllResponse<DevolucionCompra>
                {
                    statusCode = 500,
                    isExitoso = false,
        ...
        public IActionResult DeleteObject(int id)
        {
            try
            {
                ...
                if (result)
                {
                    return StatusCode(200, new GetAllResponse<DevolucionCompra>
                    {
                        statusCode = 200,
                        isExitoso = true,
