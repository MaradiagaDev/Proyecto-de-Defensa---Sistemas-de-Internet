...
namespace ApiProyectoSistemasInternet.Controllers
{
    ...
    {
        private readonly PresentacionRepository _repository = new PresentacionRepository();
        ...
        public IActionResult GetById(int id)
        {
            try
            {
                ...
                // Obtener la Presentación por ID
                ...
                if (!response.isExitoso || response.resultado == null)
                {
                    return NotFound(new
                    {
                        statusCode = 404,
                        isExitoso = false,
                        errorMessages = response.errorMessages ?? new List<object> { "Presentación no encontrada." }
                    });
                }
        ...
        [HttpPost]
        public IActionResult UpdateCreatePresentacion([FromBody] Presentacion presentacion)
        {
            try
            {
                var identity = HttpContext.User.Identity as ClaimsIdentity;
                ...
                var response = _repository.UpdateCreateObject(presentacion);

                if (response.isExitoso)
                {
                    if (response.statusCode == 201)
                    {
                        return CreatedAtAction(nameof(GetById), new { id = response.resultado.IdPresentacion }, response.resultado);
                    }
        ...
        public IActionResult GetAllObjects([FromQuery] int offSet, [FromQuery] int pageSize)
        {
            ...
            {
                return StatusCode(500, new GetAllResponse<Presentacion>
                {
                    statusCode = 500,
                    isExitoso = false,
        ...
        public IActionResult DeleteObject(int id)
        {
            try
            {
                ...
                if (result)
                {
                    return StatusCode(200, new GetAllResponse<Presentacion>
                    {
                        statusCode = 200,
                        isExitoso = true,
