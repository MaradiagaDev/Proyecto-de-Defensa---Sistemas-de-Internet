...
namespace ApiProyectoSistemasInternet.Controllers
{
    ...
    {
        private readonly DetalleCompraRepository _repository = new DetalleCompraRepository();
        ...
        public IActionResult GetById(int id)
        {
            try
            {
                ...
                // Obtener el DetalleCompra por ID
                ...
                if (!response.isExitoso || response.resultado == null)
                {
                    return NotFound(new
                    {
                        statusCode = 404,
                        isExitoso = false,
                        errorMessages = response.errorMessages ?? new List<object> { "DetalleCompra no encontrada." }
                    });
                }
        ...
        [HttpPost]
        public IActionResult UpdateCreateDetalleCompra([FromBody] DetalleCompra detalleCompra)
        {
            try
            {
                var identity = HttpContext.User.Identity as ClaimsIdentity;
                ...
                var response = _repository.UpdateCreateObject(detalleCompra);

                if (response.isExitoso)
                {
                    if (response.statusCode == 201)
                    {
                        return CreatedAtAction(nameof(GetById), new { id = response.resultado.IdDetalleCompra }, response.resultado);
                    }
        ...
        public IActionResult GetAllObjects([FromQuery] int offSet, [FromQuery] int pageSize)
        {
            ...
            {
                return StatusCode(500, new GetAllResponse<DetalleCompra>
                {
                    statusCode = 500,
                    isExitoso = false,
        ...
        public IActionResult DeleteObject(int id)
        {
            try
            {
                ...
                if (result)
                {
                    return StatusCode(200, new GetAllResponse<DetalleCompra>
                    {
                        statusCode = 200,
                        isExitoso = true,
                ...
                else
                {
                    return NotFound(new
                    {
                        statusCode = 404,
                        isExitoso = false,
                        errorMessages = new List<object> { "DetalleCompra no encontrado." }
                    });
                }
            }
            catch (Exception ex)
