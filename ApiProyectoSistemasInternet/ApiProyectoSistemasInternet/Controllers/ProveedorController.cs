...
namespace ApiProyectoSistemasInternet.Controllers
{
    ...
    {
        private readonly ProveedorRepository _repository = new ProveedorRepository();
            ...
            try
            {
                ...
                if (!response.isExitoso || response.resultado == null)
                {
                    return NotFound(new
                    {
                        statusCode = 404,
                        isExitoso = false,
                        errorMessages = response.errorMessages ?? new List<object> { "Proveedor no encontrado." }
                    });
                }
        ...
        [HttpPost]
        public IActionResult UpdateCreateProveedor([FromBody] Proveedor proveedor)
        {
            try
            {
                var identity = HttpContext.User.Identity as ClaimsIdentity;
                ...
                var response = _repository.UpdateCreateObject(proveedor);

                if (response.isExitoso)
                {
                    if (response.statusCode == 201)
                    {
                        return CreatedAtAction(nameof(GetById), new { id = response.resultado.IdProveedor }, response.resultado);
                    }
        ...
        public IActionResult GetAllObjects([FromQuery] int offSet, [FromQuery] int pageSize)
        {
            ...
            {
                return StatusCode(500, new GetAllResponse<Proveedor>
                {
                    statusCode = 500,
                    isExitoso = false,
        ...
        public IActionResult DeleteObject(int id)
        {
            try
            {
                ...
                if (result)
                {
                    return StatusCode(200, new GetAllResponse<Proveedor>
                    {
                        statusCode = 200,
                        isExitoso = true,
