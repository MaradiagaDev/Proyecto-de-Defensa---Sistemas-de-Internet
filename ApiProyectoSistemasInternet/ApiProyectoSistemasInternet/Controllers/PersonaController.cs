...
namespace ApiProyectoSistemasInternet.Controllers
{
    ...
    {
        private readonly PersonaRepository _repository = new PersonaRepository();
            ...
            try
            {
                ...
                if (!response.isExitoso || response.resultado == null)
                {
                    return NotFound(new
                    {
                        statusCode = 404,
                        isExitoso = false,
                        errorMessages = response.errorMessages ?? new List<object> { "Persona no encontrada." }
                    });
                }
        ...
        [HttpPost]
        public IActionResult UpdateCreatePersona([FromBody] Persona persona)
        {
            try
            {
                var identity = HttpContext.User.Identity as ClaimsIdentity;
                ...
                var response = _repository.UpdateCreateObject(persona);

                if (response.isExitoso)
                {
                    if (response.statusCode == 201)
                    {
                        return CreatedAtAction(nameof(GetById), new { id = response.resultado.IdPersona }, response.resultado);
                    }
        ...
        public IActionResult GetAllObjects([FromQuery] int offSet, [FromQuery] int pageSize)
        {
            ...
            {
                return StatusCode(500, new GetAllResponse<Persona>
                {
                    statusCode = 500,
                    isExitoso = false,
        ...
        public IActionResult DeleteObject(int id)
        {
            try
            {
                ...
                if (result)
                {
                    return StatusCode(200, new GetAllResponse<Persona>
                    {
                        statusCode = 200,
                        isExitoso = true,
